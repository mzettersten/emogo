---
title: "Emogo Spark Psychopathology Analysis"
...
data_path <- here("data-analysis", "data","spark-psychopathology","processed", "emogo-spark-psychopathology-alldata-anonymized.csv")
figure_path <- here("data-analysis","analysis_spark_personality","figures")
```
...
subjects_top_50 <- reward_rank %>% 
  ...
  slice_max(score_after_trial,n = 13)
...
```{r}
ratings <- d %>%
  filter(trial_index > 200 & trial_index < 265) %>%
  mutate(trial_type_cod = ifelse(trial_type == "survey-text", "emo_word", "intensity")) %>%
  select(subject, structure_condition, match_condition, trial_type_cod, simulus_image ,response) %>%
  pivot_wider(names_from = trial_type_cod, values_from = response) %>%
  extract(emo_word, into = "emo_word",
          regex = "Q0\":\"(.*)\"}") %>%
  mutate(
    emo_type = case_when(
      simulus_image== ~ "generalization block",
      simulus_image==1~"learning block"
    ))
