...
d_pers <- read_csv(here("data-analysis", "data","spark-personality","processed","emogo-spark-personality-alldata-anonymized.csv")) %>%
  ...
  #quick hack - there's a random (pure happenstance) overlap in the randomly generated subject id, remove that here
  mutate(
    subject = ifelse(subject=="p804911","p804911_2",subject)
  )

#join in demographics
d_pers_demo <- read_csv(here("data-analysis", "data","spark-personality","processed","emogo-spark-personality-demographics-data-anonymized.csv"))
d_pers <- d_pers %>%
  left_join(d_pers_demo)
...
#join in demographics
d_psych_demo <- read_csv(here("data-analysis", "data","spark-psychopathology","processed","emogo-spark-psychopathology-demographics-data-anonymized.csv")) %>%
  distinct() %>%
  #remove some duplicates or matching issues/ missing worker Ids
  filter(!(subject=="p875927" & is.na(Race))) %>%
  filter(!(subject=="p901122" & Race == "White or Caucasian")) %>%
  filter(subject!="p668856")
  
d_psych <- d_psych %>%
  left_join(d_psych_demo)
...
check_subject_id_overlap <- d %>%
  group_by(subject,structure_condition,match_condition) %>%
  tally()
...
total_time <- d %>%
  ...
  distinct() %>%
  mutate(time_mins = time_elapsed/1000/60)
#There's a few participants who have completion times that are much much too long (>60 minutes). Not in our preregistered criteria, but should probably be removed
```
...
#extract demographic information for version 1 and 2
demo_data_v12 <- d %>%
  filter(task_version %in% c("v1","v2")) %>%
  filter(trial_type %in% c("survey-text","survey-multi-select","survey-multi-choice")) %>%
  select(task_version,subject,trial_type,response) %>%
  mutate(response_raw = map(response, ~ fromJSON(.) %>% as.data.frame())) %>%
  select(-response,-trial_type) %>%
  unnest(response_raw) %>%
  select(-patterns,-strategy,-comments,-Q0) %>%
  group_by(task_version,subject) %>%
  summarise_all(first_non_na) %>%
  mutate(age=as.numeric(age)) %>%
  mutate(gender = tolower(trimws(gender))) %>%
  mutate(gender = case_when(
    gender == "m" ~ "male",
    gender %in% c("woman", "f") ~ "female",
    gender %in% c("","31","n/a") ~ NA_character_,
    TRUE ~ gender
  )) %>%
  mutate(language = case_when(
    language == "englishj" ~ "english",
    TRUE ~ tolower(trimws(language))
  )) %>%
  #define L1 English for anyone who has a match to english in the language column
  mutate(l1_english = ifelse(grepl("english",language,ignore.case = T),1,0)) 

#extract demographic information for personality and psychopathology datasets
demo_data_pers_psych <- d %>%
  filter(task_version %in% c("spark-personality","spark-psychopathology")) %>%
  select(task_version,subject,Race:Disability_5_TEXT) %>%
  distinct() %>%
  clean_names() %>%
  mutate(gender=case_when(
    current_gender %in% c("Man", "Man,Cisgender","Man,Gender fluid") ~ "male",
    current_gender == "Cisgender" ~ tolower(sexat_birth),
    current_gender %in% c("Woman","Woman,Cisgender") ~ "female",
    current_gender %in% c("Non-binary/genderqueer","Transgender,Non-binary/genderqueer") ~ "non-binary",
    current_gender %in% c("Prefer not to say/Don't know") ~ NA,
    TRUE ~ current_gender
  )) %>%
  #fix likely typo with age
...
#combine
demo_data <- demo_data_v12 %>%
  bind_rows(demo_data_pers_psych)
...
table(demo_data$gender,useNA = "always")
```
...
#note 12/17/2025: the choiceImage random intercept is a questionable modeling decision and it would likely be better to make this (1|reward_maximizing_image) (created later) 
summary(m_learn)
...
p_learning <- ggplot(subset(d,!is.na(block_trial_number)&block==1),aes(block_trial_number,as.factor(max_reward_choice),color=cur_structure_condition,fill=cur_structure_condition))+
      ...
      geom_violinh(data=subset(d,!is.na(block_trial_number)&cur_structure_condition_c==-0.5&block==1),position = position_nudge(x = 0, y = .3 ),scale="count",width=0.4,alpha=0.3,color=NA)+
      geom_violinh(data=subset(d,!is.na(block_trial_number)&cur_structure_condition_c==0.5&block==1),position = position_nudge(x = 0, y = -.3 ),scale="count",width=0.4,alpha=0.3,color=NA)+
  geom_hline(yintercept=0.25*4+1,linetype="dashed")+
      geom_smooth(data=pX,aes(y=fit*4+1,ymax=(fit+se.fit)*4+1,ymin=(fit-se.fit)*4+1),stat="identity")+
  ...
  theme(legend.position=c(0.28,0.65),legend.title=element_text(size=16),legend.text=element_text(size=14))+
      xlab("Block Trial Number")
p_learning

ggsave(here(figure_path,"learning_reward_maximizing.png"),width=9,height=6)
ggsave(here(figure_path,"learning_reward_maximizing.pdf"),width=9,height=6)
```
...
d_learning <- d_learning %>%
  ...
  mutate(rewEmoHAPPY_EX = ifelse(overall_reward_maximizing_emotion_incl_model == "model", "model",
                                 ifelse(overall_reward_maximizing_emotion_name == "exuberant" | overall_reward_maximizing_emotion_name == "happy" , "happy emotions (happy, exuberant)", "not happy emotions")) ) %>%
  mutate(
  ...
  mutate(rewEmoHAPPY_EX_c=case_when(
    rewEmoHAPPY_EX=="happy emotions (happy, exuberant)"~0.5,
    rewEmoHAPPY_EX=="not happy emotions" ~ -0.5,
    TRUE ~ NA),
    rewEmoValence_c = case_when(
      rewEmoValence == "positive emotions (calm, happy, exuberant, surprise)"~ 0.5,
      rewEmoValence == "negative emotions (anger, fear, disgust, sad)" ~ -0.5,
      TRUE ~ NA))
...
d_learning_trialwise_by_emotion_summary <- d_learning %>%
  ...
  group_by(overall_reward_maximizing_emotion_incl_model,structure_condition) %>%
  mutate(
    overall_average = mean(mean_choice)
  ) %>%
  ungroup() %>%
  mutate(overall_reward_maximizing_emotion_incl_model=fct_reorder(overall_reward_maximizing_emotion_incl_model,overall_average))

p2 <- ggplot(filter(d_learning_trialwise_by_emotion_summary,overall_reward_maximizing_emotion_incl_model!="model"),aes(test_trial_number,mean_choice,color = overall_reward_maximizing_emotion_incl_model))+
  #geom_jitter(height = .005, alpha = .05)+
  geom_point(aes(y=mean_choice),position=position_dodge(width=.9))+
  geom_errorbar(aes(y=mean_choice,ymin=mean_choice-se_choice,ymax=mean_choice+se_choice),position=position_dodge(width=.9),width=0,alpha=0.5)+
  geom_hline(yintercept=0.25, linetype="dashed")+
  geom_smooth(method = "loess")+
  xlab("Block Trial Number")+
  ylab("Probability of \nreward-maximizing choice")+
  facet_wrap(~ overall_reward_maximizing_emotion_incl_model,ncol=4)+
...
p_learning+p2+plot_annotation(tag_levels = 'A')
ggsave(here(figure_path,"learning_overall_with_emotions.png"),width=12,height=6)
ggsave(here(figure_path,"learning_overall_with_emotions.pdf"),width=12,height=6)
...
d_learning <- d_learning %>%
  ungroup() %>%
  mutate(
    overall_reward_maximizing_emotion_name_exuberant=fct_relevel(overall_reward_maximizing_emotion_name,"exuberant")
  )
m_emotion_learn <- glmer(max_reward_choice ~ overall_reward_maximizing_emotion_name_exuberant*block_trial_number_c + (1+block_trial_number_c|subject)+(1|choiceImage),data=d_learning, family=binomial,glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(m_emotion_learn)
confint(m_emotion_learn, method="Wald")
Anova(m_emotion_learn,type="III")
```

### Valence Models

No overall effect of (grouped) valence

```{r, cache=TRUE, eval=TRUE}
m_emotion_valence <- glmer(max_reward_choice ~ rewEmoValence_c*block_trial_number_c + (1|subject)+(1|choiceImage),data=d_learning, family=binomial,glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(m_emotion_valence)
confint(m_emotion_valence, method="Wald")
```

```{r, cache=TRUE, eval=TRUE}
m_emotion_happyex <- glmer(max_reward_choice ~ rewEmoHAPPY_EX_c*block_trial_number_c + (1+block_trial_number_c|subject)+(1|choiceImage),data=d_learning, family=binomial,glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(m_emotion_happyex)
confint(m_emotion_happyex, method="Wald")
```

```{r, cache=TRUE, eval=TRUE}
valence_values <- read_csv(here("data-analysis", "data","combined","BRM-emot-submit.csv")) %>%
  filter(Word %in% c("angry","disgusted","happy","sad","excited","surprised","calm","afraid")) %>%
  ...
  mutate(valence_c=valence-mean(valence),
         arousal_c=arousal-mean(arousal))
d <- d %>%
  left_join(valence_values,by=c("reward_maximizing_emotion"="short_emotion_name")) %>%
  mutate()
m_emotion_valence <- glmer(max_reward_choice ~ (valence_c+arousal_c)*block_trial_number_c + (1+block_trial_number_c|subject)+(1|choiceImage),data=filter(d,block==1&cur_structure_condition=="emotion"), family=binomial,glmerControl(optimizer="bobyqa"))
summary(m_emotion_valence)
```
...
p <- ggplot(subset(d,!is.na(block_trial_number)),aes(block_trial_number,as.factor(max_reward_choice),color=cur_structure_condition))+
  ...
  facet_wrap(~match_condition+block_name)+
      theme(legend.position=c(0.55,0.22), legend.title=element_text(size=10),legend.text=element_text(size=8),legend.background=element_blank())
p

ggsave(here(figure_path,"overall_reward_maximizing.png"),width=9,height=6)
ggsave(here(figure_path,"overall_reward_maximizing.pdf"),width=9,height=6)
```
...
p <- ggplot(subset(d,!is.na(block_trial_number)&block_name=="generalization block"),aes(block_trial_number,as.factor(max_reward_choice),color=cur_structure_condition))+
  ...
  facet_wrap(~match_condition)+
      theme(legend.position=c(0.45,0.45), legend.background = element_blank())
p

ggsave(here(figure_path,"generalization_reward_maximizing.png"),width=9,height=6)
ggsave(here(figure_path,"generalization_reward_maximizing.pdf"),width=9,height=6)
```
...
d_consolidated <- d %>%
  filter(trial_type=="explore-choice") %>%
  filter(!is.na(test_trial_number)) %>%
  select(task_version, subject,structure_condition, cur_structure_condition, match_condition,test_trial_number,block,block_name, block_trial_number, time_elapsed, rt:choice_index,max_reward_choice,reward:rewards_unadjusted,reward1:arousal,age:disability_5_text) %>%
  rename(reward_maximizing_emotion_name=Word,
         first_structure_condition=structure_condition,
         current_structure_condition=cur_structure_condition) %>%
  select(-overall_reward_maximizing_emotion_name)

#write data
write_csv(d_consolidated,here("data-analysis", "data","combined","emogo-combined-alldata-anonymized-consolidated.csv"))
```
